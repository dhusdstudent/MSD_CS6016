...
using Lab_ScaffoldingLINQ.Models;
using System.Linq;

namespace Lab_ScaffoldingLINQ.Controllers;


public class HomeController (LibraryDbContext libraryDbContextDbContext) : Controller
{
    ...
    {
        // ISDONE
        bool loginSuccessful = libraryDbContextDbContext.patrons.Any(p => p.name ==name && p.cardNum == cardnum);
        // Looks to find if there are any rows where name matches up to cardnum
    ...
    {
        var books =
            from Title in libraryDbContextDbContext.titles join Inventory in libraryDbContextDbContext.inventories
                on Title.isbn equals Inventory.isbn into IG from Inventory in IG.DefaultIfEmpty()

            join Checkedout in libraryDbContextDbContext.checkedOuts on Inventory.serial equals Checkedout.serial
                into CG from Checkedout in CG.DefaultIfEmpty()

            join Patron in libraryDbContextDbContext.patrons on Checkedout.cardNum equals Patron.cardNum 
                into PG from Patron in PG.DefaultIfEmpty()
            ...
            select new
            {
                ...
                author = Title.author,
                serial = Inventory == null ? null : Inventory.serial, 
    ...
    {
        var books =
            from checkedout in libraryDbContextDbContext.checkedOuts
            join Inventory in libraryDbContextDbContext.inventories
                on checkedout.serial equals Inventory.serial
            join Title in libraryDbContextDbContext.titles
                on Inventory.isbn equals Title.isbn
...
//ISDONE
    ...
    public ActionResult CheckOutBook(int serial)
    {
        ...
        var checkedBook = new Checkedout
        {
            cardNum = card,
            serial = serial,
        };
        
        libraryDbContextDbContext.checkedOuts.Add(checkedBook);
        libraryDbContextDbContext.SaveChanges();
    ...
    public ActionResult ReturnBook(int serial)
    {
        ...
        var checkedBook = libraryDbContextDbContext.checkedOuts.FirstOrDefault(x => x.serial == serial && x.cardNum == card);

        if (checkedBook != null)
        {

            libraryDbContextDbContext.checkedOuts.Remove(checkedBook);
            libraryDbContextDbContext.SaveChanges();
